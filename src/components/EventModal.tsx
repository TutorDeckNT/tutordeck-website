...
const EventModal = ({ isOpen, onClose }: EventModalProps) => {
  ...
  useEffect(() => {
    // This effect ensures the animation plays correctly on open/close
    setShow(isOpen);
  }, [isOpen]);

  const handleScroll = (direction: 'up' | 'down') => {
    const amount = 200; // The amount to scroll
  ...
  // Inline style to hide the scrollbar across browsers
